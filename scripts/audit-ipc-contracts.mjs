...
const entries = [...block.matchAll(/^\s*([A-Z0-9_]+):\s*'([^']+)'/gm)]
  .map((match) => ({ key: match[1], value: match[2] }));
const handledKeys = new Set(
  [...handlers.matchAll(/safeHandle\(\s*IPC_INVOKE_CHANNELS\.([A-Z0-9_]+)/g)]
    .map((match) => match[1]),
);
const handledLiterals = new Set(
  [...handlers.matchAll(/safeHandle\(\s*'([^']+)'/g)]
    .map((match) => match[1]),
);
const missingHandlers = entries
  .filter(({ key, value }) => !handledKeys.has(key) && !handledLiterals.has(value));
const unknownLiterals = [...handledLiterals]
  .filter((value) => !entries.some((entry) => entry.value === value));
